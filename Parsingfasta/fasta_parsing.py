...
class Fasta_parser():
	def __init__(self,sort_fasta,read_fasta = 'enterobacterales.fa',write_handle = 'crossrefferenced.fasta'):
		self.read_fasta = read_fasta
		self.sort_fasta = sort_fasta
		self.write_handle = write_handle

		self.names_handle  = 'names_from_'+self.read_fasta+'.txt'
		self.bacteria_names=[]
		self.unique_species=[]
		self.match_species = []
	...
	def extract_names(self):
		...
		name_file = open(self.names_handle,'w')
...
if __name__ == '__main__':
	...
	read_fasta = argv[2] # The fasta to extract names from
