...
class Fasta_parser():
	...
	def extract_names(self,read_fasta = None):
		...
		if self.read_fasta == None:
			read_handle = read_fasta
		else:
			read_handle = self.read_fasta

		names_handle = 'names_from_'+read_handle+'.txt'
		read_file = open(read_handle,'r')
	...
	def cross_ref_nrs(self,sort_fasta = None,read_fasta = None,write_handle = None):
		...
		if self.sort_fasta == None:
			sort_handle = sort_fasta
		else:
			sort_handle = self.sort_fasta
		...
		nrs_handle = 'nrs_'+write_handle
		sort_file = open(sort_handle,'r')
		crossed_file = open(nrs_handle,'w')
		match = False

		if self.bacteria_names == []:
			self.extract_names(read_handle)
